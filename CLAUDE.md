...
- **Hardlinks are deduped by `(dev, ino)`** on the generic backend. The
  ...
  a hardlink-heavy tree reports the inflated total there.
...
- Target: **within 1.3× of `dirstat-rs`** on a 1 M-file corpus,
...
- `dist-workspace.toml` configures cargo-dist (single installer: the shell
  curl-pipe).
- `.github/workflows/release.yml` is generated by `dist generate`. Don't
  hand-edit it.
- Cutting a release: bump `Cargo.toml`'s `version`, `git tag v<N>`,
  `git push --tags`.
- `cargo install clidirstat` requires a separate `cargo publish`.
